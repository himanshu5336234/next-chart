...
export const TradingViewChart = ({ themeMode, symbol = "BTCUSDT" }: { themeMode: string, ID: number; symbol: string }) => {
  ...
  useEffect(() => {
    ...
    const TradingViewWidget = new (window as any).TradingView.widget({
      ...
      toolbar_bg: themeMode === 'dark' ? '#1B1B1F' : '#FFFFFF',
      loading_screen: {
        backgroundColor: themeMode === 'dark' ? '#1B1B1F' : '#FFFFFF',
        foregroundColor: themeMode === 'dark' ? '#1B1B1F' : '#FFFFFF',
      },
      overrides: {
        'paneProperties.backgroundType': 'solid',
        'paneProperties.background': themeMode === 'dark' ? '#1B1B1F' : '#FFFFFF',
        'paneProperties.textColor': themeMode === 'dark' ? '#FFFFFF' : '#1B1B1F',
        'paneProperties.vertGridProperties.color': themeMode === 'dark' ? '#19191D' : '#F0F0F2',
        'paneProperties.horzGridProperties.color': themeMode === 'dark' ? '#19191D' : '#F0F0F2',
        'paneProperties.crossHairProperties.color': themeMode === 'dark' ? '#FFFFFF' : '#1B1B1F',
        'scalesProperties.textColor': themeMode === 'dark' ? 'white' : 'black',
        'scalesProperties.backgroundColor': themeMode === 'dark' ? '#1B1B1F' : '#FFFFFF',
